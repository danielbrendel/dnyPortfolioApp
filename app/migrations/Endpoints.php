...
/*
    Asatru PHP - Migration for Endpoints
*/

class Endpoints_Migration {
    ...
    public function up()
    {
        $this->database = new Asatru\Database\Migration('Endpoints', $this->connection);
        $this->database->drop();
        $this->database->add('id INT NOT NULL AUTO_INCREMENT PRIMARY KEY');
        $this->database->add('title VARCHAR(512) NOT NULL');
        $this->database->add('description VARCHAR(512) NOT NULL');
        $this->database->add('host VARCHAR(512) NOT NULL');
        $this->database->add('contype VARCHAR(512) NOT NULL');
